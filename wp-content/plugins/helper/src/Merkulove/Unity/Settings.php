...
final class Settings {
        ...
        <aside class="mdc-drawer">
            <div class="mdc-drawer__content">
                <nav class="mdc-list">
                    ...
                    <h6 class="mdc-list-group__subheader"><?php echo esc_html__( 'Plugin settings', 'helper' ) ?></h6>
					<?php
					...
					foreach ( $tabs as $tab => $value ) {
						...
						/** Prepare link. */
						$link = admin_url( 'admin.php?page=mdp_helper_settings&tab=' . $tab );
	...
	private function support_link() {
	    ...
	    ?>
        <hr class="mdc-list-divider">
        <h6 class="mdc-list-group__subheader"><?php echo esc_html__( 'Helpful links', 'helper' ) ?></h6>

        <a class="mdc-list-item" href="https://docs.merkulov.design/tag/helper" target="_blank">
            <i class="material-icons mdc-list-item__graphic" aria-hidden="true">collections_bookmark</i>
            <span class="mdc-list-item__text"><?php echo esc_html__( 'Documentation', 'helper' ) ?></span>
        </a>

		<?php if ( TabActivation::get_instance()->is_activated() ) : /** Activated. */ ?>
            <a class="mdc-list-item" href="https://1.envato.market/helper-support" target="_blank">
                <i class="material-icons mdc-list-item__graphic" aria-hidden="true">mail</i>
                <span class="mdc-list-item__text"><?php echo esc_html__( 'Get help', 'helper' ) ?></span>
            </a>
            <a class="mdc-list-item" href="https://1.envato.market/cc-downloads" target="_blank">
                ...
                <span class="mdc-list-item__text"><?php echo esc_html__( 'Rate this plugin', 'helper' ) ?></span>
            </a>
		<?php endif; ?>
        ...
        <a class="mdc-list-item" href="https://1.envato.market/cc-merkulove" target="_blank">
            ...
            <span class="mdc-list-item__text"><?php echo esc_html__( 'More plugins', 'helper' ) ?></span>
        </a>
		<?php
	...
	private function add_submenu_elementor( $options ) {
        ...
        add_submenu_page(
            $parent,
	        'Helper ' . esc_html__( 'Settings', 'helper' ),
	        'Helper ' . esc_html__( 'for ', 'helper' ),
	...
	private function get_current_tab(): string {
		...
		return apply_filters( 'helper_current_tab', $tab );
    ...
    private function render_nag_saved() {
        ...
        /** Render "Settings Saved" message. */
        UI::get_instance()->render_snackbar( esc_html__( 'Settings saved!', 'helper' ) );
    ...
    private function render_nag_activation() {
        ...
        if ( TabActivation::get_instance()->is_activated() ) {
            ...
            UI::get_instance()->render_snackbar( esc_html__( 'Plugin activated successfully.', 'helper' ), 'success', 5500 );
            return;

        }

	    /** Get errors from transient */
	    $error_message = esc_html__( 'Invalid purchase code.', 'helper' );
        ...
        <div class="mdc-drawer__header mdc-plugin-fixed">
            <a class="mdc-list-item mdp-plugin-title" href="#">
                <i class="mdc-list-item__graphic" aria-hidden="true">
                    <img src="<?php echo esc_attr( Plugin::get_url() . 'images/logo-color.svg' ); ?>" alt="<?php echo esc_attr( 'Helper' ) ?>">
                </i>
            ...
            <button type="submit" name="submit" id="submit" class="mdc-button mdc-button--dense mdc-button--raised">
                <span class="mdc-button__label"><?php esc_html_e( 'Save changes', 'helper' ) ?></span>
            </button>
        </div>
		<?php
	...
	public function get_options() {
        ...
        foreach ( $tabs as $tab_slug => $tab ) {
	        ...
	        $opt_name = "mdp_helper_{$tab_slug}_settings";
