...
final class TabUpdates extends Tab {
    ...
    private function __construct() {
        ...
        /** Reset Settings. */
        add_action( 'wp_ajax_check_updates_helper', [ __CLASS__, 'ajax_check_updates' ] );
	...
	public function add_settings() {
        ...
        add_settings_field( 'check_updates', esc_html__( 'Invalidate caches', 'helper' ), [ $this, 'check_updates' ], $group, $section );
        add_settings_field( 'check_ssl', esc_html__( 'SSL verification', 'helper' ), [ $this, 'check_ssl' ], $group, $section );
		add_settings_field( 'update_server', esc_html__( 'Server location', 'helper' ), [ $this, 'update_server' ], $group, $section );
    ...
    public function check_updates() {
        ...
        UI::get_instance()->render_button(
            esc_html__( 'Clear cache', 'helper' ),
	        esc_html__( 'Press to clear all nonsensitive caches and check for updates.', 'helper' ),
            'autorenew',
            [
                "name" => 'mdp_helper_' . self::TAB_SLUG . '_settings' . "[check_updates]",
                "id" => "mdp-updates-btn",
    ...
    public function check_ssl() {
        ...
        UI::get_instance()->render_switcher(
            ...
            esc_html__( 'SSL verification', 'helper' ),
            esc_html__( 'Enable to activate the SSL check when receiving updates', 'helper' ),
            [
                "name" => 'mdp_helper_' . self::TAB_SLUG . '_settings' . "[check_ssl]",
            ]
	...
	public function update_server(): void {
		...
		UI::get_instance()->render_select(
			...
			esc_html__( 'Location', 'helper' ),
			esc_html__( 'Switch if you are unable to activate the plugin with your purchase code.', 'helper' ),
			[
				"name" => 'mdp_helper_' . self::TAB_SLUG . '_settings' . "[update_server]",
			]
		);
    ...
    public static function ajax_check_updates() {
        ...
        if ( empty( $_POST['checkUpdates'] ) ) {
	        wp_send_json( esc_html__( 'Wrong Parameter Value.', 'helper' ) );
        }
