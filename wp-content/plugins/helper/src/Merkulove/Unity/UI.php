...
final class UI {
                ...
                <div class="mdc-snackbar__actions">
					...
					<?php if ( $closeable ) : ?>
                        <button class="mdc-icon-button mdc-snackbar__dismiss material-icons"
                                title="<?php esc_html_e( 'Dismiss', 'helper' ); ?>" type="button">close
                        </button>
					<?php endif; ?>
                </div>
            </div>
	...
	public function render_icon( $value, string $label = '', string $helper_text = '', array $attributes = [], array $meta = [] ) {
		...
		$icons_json = wp_json_encode( $icons_arr );
        ...
        <div <?php echo ( $class ) ? 'class="' . esc_attr( $class ) . '"' : ' '; ?>>
            ...
            <div class="mdc-icon-field-img-box <?php if ( $src ) : ?>mdc-with-image<?php endif; ?>">
                ...
                ><?php esc_html_e( 'Icon Library', 'helper' ); ?></button>
                <button class="mdc-icon-field-upload-btn mdc-button mdc-button--outlined mdc-ripple-upgraded"><?php esc_html_e( 'Upload SVG', 'helper' ); ?></button>
            </div>
	...
	public function render_media_library( $value, string $helper_text = '', array $attributes = [] ) {
		...
		$image = intval( $value ) > 0 ?
			...
			wp_sprintf(
				...
				$id . '-preview-image',
				esc_html__( 'Preview image', 'helper' )
			);
		...
		echo wp_sprintf(
			...
			$image,
			esc_attr__( 'Remove', 'helper' ),
			esc_attr( $id ),
			esc_html__( 'Media library', 'helper' )
		);
        ...
        <div class="mdp-dnd">
            ...
            <div class="mdc-text-field mdc-input-width mdc-text-field--outlined mdc-hidden">
                ...
                <input type="text"
                       ...
                       name="mdp_helper_<?php echo esc_attr( $tab ); ?>_settings[<?php echo esc_attr( $key ); ?>]"
                       id="mdp-helper-settings-dnd-<?php echo esc_attr( $key ); ?>"
                       value="<?php echo esc_attr( $value ); ?>"
                >
                <div class="mdc-notched-outline mdc-notched-outline--upgraded mdc-notched-outline--notched">
                    <div class="mdc-notched-outline__leading"></div>
                    <div class="mdc-notched-outline__notch">
                        <label for="mdp-helper-settings-dnd-<?php echo esc_attr( $key ); ?>"
            ...
            <div id="mdp-<?php echo esc_attr( $key ); ?>-drop-zone"
				...
				<?php if ( $key_exist ) : ?>
                    <span class="material-icons">check_circle_outline</span><?php esc_html_e( 'File exist', 'helper' ); ?>
                    <span class="mdp-drop-zone-hover"><?php esc_html_e( 'Drop file here or click to upload', 'helper' ); ?></span>
				<?php else : ?>
                    <span class="material-icons">cloud</span><?php esc_html_e( 'Drop file here or click to upload.', 'helper' ); ?>
				<?php endif; ?>
            </div>
			<?php if ( $key_exist ) : ?>
                <div class="mdp-messages mdc-text-field-helper-line mdc-text-field-helper-text mdc-text-field-helper-text--persistent">
					<?php esc_html_e( 'Drag and drop or click on the form to replace file. |', 'helper' ); ?>
                    <a href="#" class="mdp-reset-key-btn"><?php esc_html_e( 'Reset', 'helper' ); ?></a>
                </div>
			<?php else : ?>
                <div class="mdp-messages mdc-text-field-helper-line mdc-text-field-helper-text mdc-text-field-helper-text--persistent">
					<?php esc_html_e( 'Drag and drop or click on the form to add file', 'helper' ); ?>
                </div>
			<?php endif; ?>
        ...
        <div class="mdp-dnd mdp-file">
			...
			echo wp_sprintf(
				'<div class="mdc-text-field mdc-input-width mdc-text-field--outlined mdc-hidden">
					<input  type="text"
							class="mdc-text-field__input mdp-drop-zone-input"
							name="mdp_helper_%1$s_settings[%2$s]"
							id="mdp-helper-settings-dnd-%2$s"
							value="%3$s"
					>
					<div class="mdc-notched-outline mdc-notched-outline--upgraded mdc-notched-outline--notched">
						<div class="mdc-notched-outline__leading"></div>
						...
						<div class="mdc-notched-outline__notch">
							<label for="mdp-helper-settings-dnd-%2$s" class="mdc-floating-label mdc-floating-label--float-above">%4$s</label>
						</div>
						<div class="mdc-notched-outline__trailing"></div>
					</div>
				</div>',
				...
				isset( $field['label'] ) ? esc_html( $field['label'] ) : esc_html__( 'File', 'helper' )
			);
			...
			if ( $file_exist ) :
				...
				echo wp_sprintf(
					...
					esc_html__( 'File exist', 'helper' ),
					esc_html__( 'Drop file here or click to upload', 'helper' )
				);
			...
			if ( $file_exist ) :
				...
				echo wp_sprintf(
					...
					esc_html__( 'Drag and drop or click on the form to replace file.', 'helper' ),
					esc_html__( 'Reset', 'helper' ),
					$field['description'] ?? ''
				);
				...
				echo wp_sprintf(
					...
					esc_html__( 'Drag and drop or click on the form to add file.', 'helper' ),
					$field['description'] ?? '',
					isset( $field['file_types'] )
						? esc_html__( 'Allowed file types:', 'helper' ) . ' <b>' . esc_attr( $file_types ) . '</b>' : ''
				);
	...
	private function get_control_id( $key, $tab_slug, $field ) {
		...
		$id = 'mdp_helper_' . $tab_slug . '_settings_' . $key;

		return apply_filters( 'mdp_helper_settings_control_id', $id, $key, $tab_slug, $field );
	...
	private function get_control_name( $key, $tab_slug, $field ) {
		...
		$name = 'mdp_helper_' . $tab_slug . '_settings[' . $key . ']';

		return apply_filters( 'mdp_helper_settings_control_name', $name, $key, $tab_slug, $field );
	...
	private function get_sides_values( $options, $slug, array $default = array() ): array {
		...
		return apply_filters( 'helper_settings_side_values', $values, $slug );
	...
	private function get_sides_unit( $options, $slug, array $default = array() ) {
		...
		return apply_filters( 'helper_settings_sides_unit', $unit, $slug );
	...
	private function get_sides_linked( $options, $slug, array $default = array() ) {
		...
		return apply_filters( 'helper_settings_sides_linked', $linked, $slug );
