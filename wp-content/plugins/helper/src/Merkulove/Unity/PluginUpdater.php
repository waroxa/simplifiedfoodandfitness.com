...
final class PluginUpdater {
     ...
     * @var string
	 **/
	public $opt_plugin_info = 'mdp_helper_plugin_info';
	...
	public function plugin_info( $res, $action, $args ) {
		...
		if ( 'helper' !== $args->slug ) { return false; }
	...
	private function get_plugin_information() {
        ...
        if ( ! is_array( $plugin_info ) ) {
	        ...
	        if ( ! $plugin_info->mdp_helper_plugin_info ) {
		        return false;
	        } else {
		        $plugin_info = json_decode( $plugin_info->mdp_helper_plugin_info->body, false );
	        }
        }
	...
	public function show_plugin_update_message( $plugin_data, $r ) {
		...
		if ( ! TabActivation::get_instance()->is_activated() ) {
			...
			echo ' <a href="https://1.envato.market/helper" target="_blank">';
