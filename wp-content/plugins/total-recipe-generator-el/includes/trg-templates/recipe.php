...
echo '<div class="trg-recipe elementor-widget-heading" itemscope itemtype="http://schema.org/Recipe">';
    ...
    <div class="ingredients-section clearfix">
		...
		if ( $use_ing_alt && isset( $ing_alt ) && ! empty ( $ing_alt ) ) {
			$ingredients = $method_list = explode( "|", $ing_alt );
        ...
        elseif ( isset( $ingredients ) && is_array( $ingredients ) ) {
	...
	if ( ( isset( $methods ) && is_array( $methods ) ) || isset( $methods_alt ) ) {
	?>
        <div class="method-section clearfix">
			...
			// Alternate methods via dynamic field
			if ( $use_method_alt && isset( $methods_alt ) && ! empty ( $methods_alt ) ) {

				$method_list = explode( "|", $methods_alt );
				if ( isset( $method_list ) && is_array( $method_list ) ) {
					foreach( $method_list as $method ) {
						...
						$ins_json['text'] = strip_tags( $method );
			...
			// Native repeater field
			elseif ( isset( $methods ) && is_array( $methods ) ) {
